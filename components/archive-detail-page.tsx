...
export function ArchiveDetailPage({
  ...
  slug?: keyof typeof archiveDetails;
        ...
        <div className="relative mx-auto flex w-full max-w-7xl flex-col px-5 pb-24 pt-5 sm:px-8 lg:px-12">
          ...
          <section className="flex min-h-[52vh] items-end py-18 sm:py-24">
            ...
            >
              ...
              <p className="max-w-2xl text-base leading-8 text-muted sm:text-lg">{record.shortSummary}</p>
            </motion.div>
          </section>

          {record.coverImageUrl ? (
            <section className="border-t border-white/5 pb-6 pt-8 sm:pt-12">
              <img src={record.coverImageUrl} alt={record.title} className="max-h-[36rem] w-full rounded-[2rem] object-cover" />
            </section>
          ) : null}
          ...
          <section className="border-t border-white/5 py-20 sm:py-28">
            <div className="rounded-[2rem] border border-white/8 bg-surface/70 px-6 py-3 backdrop-blur-sm sm:px-8">
              {[
                { label: "Overview", value: record.shortSummary },
                { label: "Status", value: record.status },
                { label: "Date", value: formatArchiveDate(record.archiveDate) },
                { label: "Category", value: record.category },
                { label: "Full Description", value: record.fullDescription },
              ].map((item) => (
                <motion.div
                  key={item.label}
