...
export const metadata: Metadata = {
  title: "new curated item | hisarchives.xyz",
  robots: { index: false, follow: false },
};

export default async function CuratedNewRoute() {
        ...
        <div className="mx-auto max-w-4xl rounded-[1.8rem] border border-white/8 bg-white/[0.03] px-6 py-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-light tracking-[0.18em]">New Curated Item</h1>
            </div>
            <AdminSignOutButton />
          </div>
          <p className="mt-6 text-sm leading-7 text-white/65">
            Set the Supabase admin environment variables before creating curated items.
          </p>
        </div>
      </main>
    );
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        <header className="flex flex-wrap items-center justify-between gap-4 rounded-[1.8rem] border border-white/8 bg-white/[0.03] px-5 py-4 shadow-[0_22px_70px_rgba(0,0,0,0.28)]">
          <div className="space-y-1">
            <p className="text-[0.68rem] uppercase tracking-[0.45em] text-white/50">HisArchives Admin</p>
            <h1 className="text-3xl font-light tracking-[0.18em]">New Curated Item</h1>
            <p className="text-sm text-white/60">Dedicated page editor. No modal, no overlay.</p>
          </div>
        ...
        <section className="rounded-[1.8rem] border border-white/8 bg-white/[0.03] p-5 shadow-[0_18px_60px_rgba(0,0,0,0.2)] sm:p-6">
          <CuratedEditorForm
            mode="create"
            actionPath="/test/curated/submit"
            values={{
              title: "",
              category: "",
              shortDescription: "",
