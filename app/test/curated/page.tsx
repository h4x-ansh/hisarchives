...
import { listCuratedEntries } from "@/lib/curated/repository";

export const metadata: Metadata = {
  title: "curated admin | hisarchives.xyz",
  robots: { index: false, follow: false },
};

function CuratedThumbnail({ imageUrl, title }: { imageUrl: string; title: string }) {
  if (!imageUrl) {
    return (
      <div className="flex h-full items-center justify-center bg-[linear-gradient(180deg,#ece2d0,#d8c4a8)] text-xs uppercase tracking-[0.35em] text-[#6b6052]">
        No image
      </div>
    );
  }
...
export default async function CuratedAdminRoute() {
        ...
        <div className="mx-auto max-w-4xl rounded-[1.8rem] border border-white/8 bg-white/[0.03] px-6 py-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-light tracking-[0.18em]">Curated</h1>
            </div>
            <AdminSignOutButton />
          </div>
          <p className="mt-6 text-sm leading-7 text-white/65">
            Configure `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` to use the curated CMS.
          </p>
        </div>
      </main>
    );
  ...
  const entries = await listCuratedEntries({ includeDrafts: true });
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        <header className="flex flex-wrap items-center justify-between gap-4 rounded-[1.8rem] border border-white/8 bg-white/[0.03] px-5 py-4 shadow-[0_22px_70px_rgba(0,0,0,0.28)]">
          <div className="space-y-1">
            <p className="text-[0.68rem] uppercase tracking-[0.45em] text-white/50">HisArchives Admin</p>
            <h1 className="text-3xl font-light tracking-[0.18em]">Curated</h1>
            <p className="text-sm text-white/60">Create, edit, publish, and delete curated items.</p>
          </div>
        ...
        {entries.length ? (
              ...
              >
                <div className="overflow-hidden rounded-[1.25rem] border border-white/8 bg-white/5">
                  <div className="aspect-[4/3]">
                    <CuratedThumbnail imageUrl={entry.imageUrl} title={entry.title} />
                  </div>
                </div>
                ...
                <div className="flex min-w-0 flex-col gap-4">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-white/55">
                        {entry.displayOrder} - {entry.category}
                      </p>
                    </div>

                    <div className="flex flex-wrap items-center gap-2">
                      <Link
                        href={`/test/curated/edit/${entry.id}`}
                        className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-xs uppercase tracking-[0.32em] text-white/80 transition hover:bg-white/[0.08]"
                      >
                        Edit
                      </Link>
                      <form action="/test/curated/submit" method="post">
                  ...
                  <p className="max-w-3xl text-sm leading-7 text-white/65">{entry.shortDescription}</p>
                </div>
              </article>
            ))}
          </section>
          ...
          <div className="rounded-[1.8rem] border border-white/8 bg-white/[0.03] px-6 py-10 text-center text-white/65">
            No curated items yet. Create the first one from the new curated page.
          </div>
        )}
      </div>
    </main>
